...
public class ServiceDistribuirTurmas extends AbstractService<DistribuirTurmas, RespostaDistribuirTurmas> {
   ...
   public RespostaDistribuirTurmas processa(DistribuirTurmas solicitacao) throws NegocioException {
      ...
      for (String d : this.turmaDao.getAllNomesDocentes(solicitacao.getIdPeriodo(), solicitacao.getIdPredio())) {

         // Obter as turmas por docentes
         for (Turma t1 : this.turmaDao.getTurmasPorDocente(d, solicitacao.getIdPeriodo(), solicitacao.getIdPredio())) {

            // Eliminar as turmas que tiver sem docente definido
            if (!t1.getDocente().replaceAll(" ", "").equals("ADEFINIRDOCENTE")) {
               ...
               if (auxiliar.validarHorarioComSabado(t1.getHorario())) {
                  ...
                  for (Turma tu : this.turmaDao.getTurmasPorDocente(d, solicitacao.getIdPeriodo(), solicitacao.getIdPredio())) {
      ...
      for (String d : this.turmaDao.getAllNomesDocentes(solicitacao.getIdPeriodo(), solicitacao.getIdPredio())) {
         ...
         for (Turma t1 : this.turmaDao.getTurmasPorDocente(d, solicitacao.getIdPeriodo(), solicitacao.getIdPredio())) {
            ...
            if (!t1.getDocente().replaceAll(" ", "").equals("ADEFINIRDOCENTE") && t1.getDistribuir() == true
               && auxiliar.validarHorarioComSabado(t1.getHorario())) {
               ...
               for (Sala s : this.salaDao.getAllDiponiveisParaDistribuicao(solicitacao.getIdPredio())) {
      ...
      for (Turma t : this.turmaDao.getTurmasNaoDistribuidas(solicitacao.getIdPeriodo(), solicitacao.getIdPredio())) {
         if (!this.disponibilidadeSalaDao.verificarTurmaTemSala(t.getId()) && auxiliar.validarHorarioComSabado(t.getHorario())) {
            ...
            if (periodoLetivoAnterior != null) {
               ...
               if (turmaAnteriorSemelhante != null && !this.disponibilidadeSalaDao.verificarTurmaTemSala(t.getId())) {
